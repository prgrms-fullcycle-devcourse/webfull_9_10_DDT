...
export function SemiResult() {
  ...
  const isNoDisruption = !!result?.allClear;
          ...
          <div className='w-full divide-y divide-slate-800/40 overflow-hidden rounded-2xl border border-slate-900 bg-[#151926]'>
            {rankedMembers.map((member) => {
              ...
              if (isTopRank && !isNoDisruption) {
                if (member.rank === 1) rankColor = 'font-bold text-[#F85A5A]';
                ...
                >
                  <div className='flex items-center gap-3.5'>
                    {member.isAllClear || isNoDisruption ? (
                      ...
                      <span className={`w-4 text-center text-xs ${rankColor}`}>
                        {member.rank}
                      </span>
                    )}
                    ...
                    <div className='flex items-center gap-1.5'>
                      <span
                        className={`text-xs font-semibold tracking-tight ${
                          member.gaveUpAt ? 'text-[#F85A5A]' : 'text-slate-200'
                        }`}
                      >
                        {member.nickname}
                        {member.isHost && ' (방장)'}
