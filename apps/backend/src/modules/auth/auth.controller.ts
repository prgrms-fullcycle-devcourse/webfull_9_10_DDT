...
export class AuthController {
  ...
  @ApiOperation({ 
    ...
    description: '구글 인증이 완료된 후 구글 서버가 백엔드로 리다이렉트하는 주소입니다. 팝업창을 닫고 프론트엔드로 토큰을 전송합니다.' 
  })
  @Get('google/callback')
  @UseGuards(AuthGuard('google'))
  ...
  @ApiOperation({ 
    ...
    description: '로그인 없이 서비스를 이용하는 유저에게 게스트용 JWT 토큰과 식별자를 발급합니다.' 
  })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiOperation({ 
    ...
    description: '신규 가입 유저가 필수 약관 3종에 모두 동의했을 때 호출하여 DB 상태를 업데이트합니다.' 
  })
  @ApiBody({ type: AgreeTermsDto })
  @ApiResponse({ status: 200, description: '약관 동의 완료' })
  @ApiResponse({ status: 400, description: '필수 항목 미동의 (INVALID_REQUEST)' })
  @ApiResponse({ status: 401, description: '인증 토큰 누락/만료 (UNAUTHORIZED)' })
  @ApiResponse({ status: 409, description: '이미 약관 동의가 완료된 계정 (ALREADY_AGREED)' })
  @UseGuards(AuthGuard('jwt')) 
  @Post('terms')
  async agreeTerms(@Req() req, @Body() body: AgreeTermsDto) {
    try {
  ...
  @ApiOperation({ 
    summary: '로그아웃 (토큰 무효화)', 
    description: '현재 사용 중인 JWT 토큰을 Redis 블랙리스트에 등록하여 강제로 무효화시킵니다.' 
  })
  @ApiResponse({ status: 200, description: '로그아웃 성공' })
  @ApiResponse({ status: 401, description: '유효하지 않은 인증 토큰 (UNAUTHORIZED)' })
  @UseGuards(AuthGuard('jwt'))
  @Post('logout')
